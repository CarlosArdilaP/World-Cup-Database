...
echo "$($PSQL "SELECT winner.name AS equipo
                FROM games
                INNER JOIN teams AS winner ON games.winner_id = winner.team_id
                WHERE round = 'Eighth-Final' AND year = 2014
                UNION ALL
                SELECT loser.name AS equipo
                FROM games
                INNER JOIN teams AS loser ON games.opponent_id = loser.team_id
                WHERE round = 'Eighth-Final' AND year = 2014
                ORDER BY equipo")"
